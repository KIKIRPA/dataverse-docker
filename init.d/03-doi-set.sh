...
if [ "${doi_authority}" ]; then
    if [ "${doi_protocol}" = "hdl" ]; then
      ...
      asadmin --user=${ADMIN_USER} --passwordfile=${PASSWORD_FILE} create-jvm-options "-Ddataverse.fqdn\=${hostname}"
      asadmin --user=${ADMIN_USER} --passwordfile=${PASSWORD_FILE} create-jvm-options "-Ddataverse.siteUrl\=http\\:\/\/${hostname}"
