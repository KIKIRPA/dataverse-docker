...
if [ "${doi_authority}" ]; then
    if [ "${doi_protocol}" = "hdl" ]; then
      ...
      asadmin --user=${ADMIN_USER} --passwordfile=${PASSWORD_FILE} create-jvm-options "-Ddataverse.fqdn\=localhost\\:8080"
      asadmin --user=${ADMIN_USER} --passwordfile=${PASSWORD_FILE} create-jvm-options "-Ddataverse.siteUrl\=http\\:\/\/localhost\\:8080"
